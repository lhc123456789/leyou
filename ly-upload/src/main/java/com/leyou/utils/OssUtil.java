...
public class OssUtil {
    ...
    private static void paramInit() {
        if(OSS_END_POINT == null){
            OSS_END_POINT = PropertiesUtils.getProperty("ly.upload.endpoint");
        }
        if(OSS_ACCESS_KEY_ID == null){
            OSS_ACCESS_KEY_ID = PropertiesUtils.getProperty("ly.upload.accessKeyId");
        }
        if(OSS_ACCESS_KEY_SECRET == null){
            OSS_ACCESS_KEY_SECRET = PropertiesUtils.getProperty("ly.upload.accessKeySecret");
        }
        if(OSS_BUCKET_NAME == null){
            OSS_BUCKET_NAME = PropertiesUtils.getProperty("ly.upload.bucketName");
        }
    }
    ...
    public enum FileDirType {
        ...
        ARTICLE_IMAGES("images"), ARTICLE_VIDEO("video"), CAROUSEL("carousel"), MENU("menu");
     ...
     * @return
     */
    public static String getFileUrl(String fileUrl, FileDirType fileDirType) {
    ...
    public static String upload(MultipartFile file, FileDirType fileDirType) {
        ...
        System.out.println("上传OSS执行时间：" + (endTime - startTime) + "ms");    //输出程序运行时间
        return fileOssUrl;
    }
